...
如果你是 Linux 或是 MacOS 系統，通常都有內建 OpenSSH 客戶端，可以直接跳至 以 [Step 2 --- 產生 ssh key pairs] 的章節，Windows 與 Linux 使用 OpenSSH Utility 產生 ssh key pairs 的流程是一樣的。
...
在 Windows 要產生 ssh key pairs 有兩種辦法，第一種是經由內建的 OpenSSH Client 產生，另一種是經由 PuTTY 所附的工具產生 (可以參考附錄的[使用 PuTTY 連線至 OpenSSH 伺服器][PuTTY])。

::: {.infobox .info}
Linux 的使用者可以使用套件管理程式安裝 `openssh-server`，如果系統是則指令為：
