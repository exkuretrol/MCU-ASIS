...
在來回到虛擬主機安裝 net-tools

``` {.bash .prefixed}
sudo apt install net-tools
```

確認目前的裝置名稱，可以看到 `08:00:27:59:b3:11` 與剛才新增的介面卡 mac 位址相同。`enp0s8` 即為剛才插入的新的「僅限主機」網路卡。
...
我們可以透過修改 netplan 的設定將 dhcp 伺服器打開。
...
後續我們就可以透過虛擬主機的 ip 位址 `192.168.56.101` 存取上面的服務。


## OpenSSH {#openssh}

在前第一章的安裝過程中，也有碰到 OpenSSH 的小節，不過那並不是給第一次要使用 OpenSSH 的使用者用的，而是給已經有在使用 SSH 金鑰的使用者看的。在這個小節會逐步解釋 OpenSSH 的使用方法，從 OpenSSH 客戶端安裝、產生 SSH 金鑰再來是連線。
...
安裝完成後，你可以打開命令提示字元或是 PowerShell 輸入指令 `ssh` 來確認是否安裝完成。
...
這一步再詢問你需不需要將 SSH key pairs 加密，如果加密了，每次連線都需要輸入密碼。如果怕麻煩不想設定密碼就留空按 Enter↩ 進入下一步。
...
-   這裡的 `$env:USERPROFILE` 為 Windows 內建環境變數，會根據現在的使用者變更。如果我的使用者名稱為 `qaz855175b` 那 `$env:USERPROFILE` 就會產生 `C:\Users\qaz855175b\` 的字串，當然你可以直接把這行指令的公鑰路徑替換為 `C:\Users\qaz855175b\.ssh\id_rsa.pub`

-   `id_rsa.pub` 為剛剛產生的公鑰名稱

-   這裡的 `asis` 為 [user-configuration] 的預設使用者

-   `192.168.56.101` 為我虛擬主機的 ip 位址，查詢 ip 位址可以先手動登入虛擬主機後，用 `hostname -I` 或是 `ip addr show` 指令查詢

::: {.infobox .info}
其實 Linux 的 `ssh-copy-id`、和第一章的從 Github 匯入 public key `ssh-import-id-gh` 與這裡的 `cat file >> ~/.ssh/authorized_keys` 都是在做同一件事情，就是把公鑰貼到遠端伺服器的使用者家目錄的 `authorized_keys` 檔案裡面。
:::
...
使用 OpenSSH Client 產生的 pair keys 可以直接使用這個方法連線。這裡介紹幾個常常與 ssh 一起出現的參數：
