# RStudio & RShiny Server {#rstudio-and-rshiny-server}

本章節會說明怎麼安裝 RStudio 與 RShiny Server 到系統上。

## 事前準備 {#rstudio-shiny-preparation}
...
### 確認 R Base 是否成功安裝 {#verify-r-base-installation}
...
### 安裝 RStudio Server {#install-rstudio-server}
...
### 設定 Apache 的反向代理 {#configure-rstudio-apache-reverse-proxy}
...
### 安裝 R Shiny Server {#install-r-shiny-server}
...
### 設定 Apache 的反向代理 {#configure-shiny-apache-reverse-proxy}
...
### R Shiny Server 進階設定 {#r-shiny-server-advanced-settings}
...
::: {.infobox .rmdcaution}
這裏要至下一節先建立使用者群組後再重新啟動 R Shiny Server。因為設定檔中設有使用者群組（`stu`、`teacher`）而沒有創建的話，重新啟動 R Shiny Server 會因為找不到該使用者群組而出現錯誤。
:::

### 使用使用者測試設定是否有效 {#user-testing-to-verify-settings}
