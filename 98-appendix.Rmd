...
check_depts_dir() {
	if [ -d ${depts_dir} ]; then
		...
		do
			...
			if [ $count -lt 1 ]; then
				echo "請在在檢查一次 ./${depts_dir}/ 內是否含有正確格式的學號檔案！"
				echo "有問題的檔案: $file"
				exit 1
			else
		...
		echo "系資料夾 ${depts_dir} 不存在！請再確認一次。"
		exit 1;
	fi
}
...
make_pass_dir() {
	if [ ! -d ${pass_dir} ]; then
		echo "存放密碼 ${pass_dir} 資料夾不存在，將建立 ${pass_dir} 資料夾"
		mkdir ${pass_dir}
	else
		read -p "存放密碼 ${pass_dir} 資料夾已存在！是否要將其移除後繼續建立帳號? (yes/No)" yn
		case $yn in
			[Yy]* ) 
				rm -rf ${pass_dir}
				make_pass_dir
...
display_help() {
	...
	echo 
	echo "選項: "
	echo "	-np, --dont-generate-password	不產生隨機的使用者密碼，以學生學號取代"
	echo
	echo "此腳本被用來: "
